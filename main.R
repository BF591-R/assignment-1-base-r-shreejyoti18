...
#Here we are calculating median by row; where x is an input, 1 is a margin and median is the function to appy the imput data
...
summarize_rows <- function(x, fn, na.rm=FALSE) {
    apply(x,1, function(row) fn(row, na.rm =TRUE))
}
